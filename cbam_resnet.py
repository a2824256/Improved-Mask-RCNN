...
class ResNet(object):
    ...
    def channel_attention(self, input_feature, index, ratio=0.125):
        ...
        # avg path
        avg_path = fluid.layers.pool2d(name='cbam_avg_pooling_' + str(index), input=input_feature, global_pooling=True,
                                       pool_type='avg', pool_padding='SAME')
        avg_path = fluid.layers.fc(name='cbam_avg_fc1_' + str(index), input=avg_path, size=size,
                                   param_attr=ParamAttr(name='fc_share_1_' + str(index), trainable=True), act='relu')
        avg_path = fluid.layers.fc(name='cbam_avg_fc2_' + str(index), input=avg_path, size=channel,
                                   param_attr=ParamAttr(name='fc_share_2_' + str(index), trainable=True), act='relu')
        avg_path = fluid.layers.reshape(x=avg_path, shape=[-1, channel, 1, 1])
        # max path
        max_path = fluid.layers.pool2d(name='cbam_max_pooling_' + str(index), input=input_feature, global_pooling=True,
                                       pool_type='max', pool_padding='SAME')
        max_path = fluid.layers.fc(name='cbam_max_fc1_' + str(index), input=max_path, size=size,
                                   param_attr=ParamAttr(name='fc_share_1_' + str(index), trainable=True), act='relu')
        max_path = fluid.layers.fc(name='cbam_max_fc2_' + str(index), input=max_path, size=channel,
                                   param_attr=ParamAttr(name='fc_share_2_' + str(index), trainable=True), act='relu')
        ...
        for i in range(count):
            ...
            if i % nonlocal_mod == nonlocal_mod - 1:
                conv = add_space_nonlocal(conv, dim_in, dim_in,
                                          ...
                                          int(dim_in / 2))
        if self.stage_num != 5:
            conv = self.cbam_module(conv, self.stage_num)
            conv = fluid.layers.elementwise_add(x=input, y=conv, act='relu')
        return conv
    ...
    def __call__(self, input):
        ...
        if not severed_head:
            ...
            cbam_res = self.cbam_module(res, 1)
            res = fluid.layers.elementwise_add(x=input, y=cbam_res, act='relu')
