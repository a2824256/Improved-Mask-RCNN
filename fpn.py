...
class FPN(object):
    ...
    def channel_attention(self, input_feature, index, ratio=0.125):
        ...
        avg_path = fluid.layers.fc(name='cbam_avg_fc1_' + str(index), input=avg_path, size=size, bias_attr=ParamAttr(name='fc_share_bias_1_' + str(index)), param_attr=ParamAttr(name='fc_share_1_' + str(index),  trainable=True), act='relu')
        avg_path = fluid.layers.fc(name='cbam_avg_fc2_' + str(index), input=avg_path, size=channel, bias_attr=ParamAttr(name='fc_share_bias_2_' + str(index)), param_attr=ParamAttr(name='fc_share_2_' + str(index), trainable=True), act='relu')
        ...
        max_path = fluid.layers.fc(name='cbam_max_fc1_' + str(index), input=max_path, size=size, bias_attr=ParamAttr(name='fc_share_bias_1_' + str(index)), param_attr=ParamAttr(name='fc_share_1_' + str(index), trainable=True), act='relu')
        max_path = fluid.layers.fc(name='cbam_max_fc2_' + str(index), input=max_path, size=channel, bias_attr=ParamAttr(name='fc_share_bias_2_' + str(index)), param_attr=ParamAttr(name='fc_share_2_' + str(index), trainable=True), act='relu')
        ...
        # output = fluid.layers.elementwise_add(avg_path, max_path)
    ...
    def spatial_attention(self, channel_feature, index):
        ...
        output = fluid.layers.conv2d(name='cbam_conv2d_' + str(index), input=concat_feature, num_filters=1, filter_size=(3, 3), padding="SAME", act="sigmoid")
    ...
    def pam_module(self, input, index):
        channel_refined_feature = self.channel_attention(input, index)
        spatial_feature = self.spatial_attention(channel_refined_feature, index)
        add_feature = fluid.layers.elementwise_add(channel_refined_feature, spatial_feature, act='sigmoid')
        return fluid.layers.elementwise_mul(add_feature, input)

    # def cbam_module(self, input, index):
    #     channel_refined_feature = self.channel_attention(input, index)
    #     spatial_refined_feature = self.spatial_attention(channel_refined_feature, index)
    #     add_feature = fluid.layers.elementwise_add(input, spatial_refined_feature, act='relu')
    #     return add_feature
    ...
    def get_output(self, body_dict):
        ...
        for i in range(num_backbone_stages):
            ...
            print(1)
            fpn_output = self.pam_module(fpn_output, fpn_name)
        ...
        if not self.has_extra_convs and self.max_level - self.min_level == len(
            ...
            print(2)
            body_top_extension = self.pam_module(body_top_extension, body_top_name)
            fpn_dict[body_top_name + '_subsampled_2x'] = body_top_extension
            # print("add " + body_top_name + '_subsampled_2x')
            # print(fpn_dict.keys())
        ...
        if self.has_extra_convs and self.max_level > highest_backbone_level:
            ...
            for i in range(highest_backbone_level + 1, self.max_level + 1):
                ...
                # pam 5
                print(3)
                fpn_blob = self.pam_module(fpn_blob, fpn_name)
